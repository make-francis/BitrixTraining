			...
			foreach($productRow as $product_key => $prod_value)
			{
				switch($product_key){
					case "DISCOUNT_PRICE":
						...
						$arData["PROD_".$product_key."_AMOUNT_".$key] = $discount;
...
/**/
  ...
  if ( ($request == "render") && ($temp_id != "") ) {
      ...
      $path = $_SERVER["DOCUMENT_ROOT"]."/crm/invoice/InvoiceTemplate.docx"; //$_SERVER["DOCUMENT_ROOT"].$f['SRC'];
      ...
      if (file_exists($fileDiskPath)) {
        ...
        if(($file_format === 'docx') || ($file_format === 'pdf') ):
          ...
          if(!is_null($subFolderModelCode = $folder->addSubFolder(array('NAME' => $invoice_id_folder, 'CREATED_BY' => $userId)))){
