...
if (strtolower($_REQUEST['lang']) == 'de')
    define('TEMPLATE_SOURCE', $_SERVER["DOCUMENT_ROOT"]."/crm/templates/Quote-de.docx"); // path for source templates
else
    define('TEMPLATE_SOURCE', $_SERVER["DOCUMENT_ROOT"]."/crm/templates/Quote.docx"); // path for source templates
...
if ( ($request == "render") && ($templateId != "") ) {
    ...
    // $uploadFile = $_SERVER["DOCUMENT_ROOT"]."/crm/templates/QuoteTemplate2.docx"; 
    ...
    if (file_exists($fileDiskPath)) {
        ...
        if(in_array($file_format, array('docx', 'pdf'))):
            ...
            if('pdf' == $file_format):
                ...
                $fileArray = \CFile::MakeFileArray($pdfDiskPath);
