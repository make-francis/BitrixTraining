...
<h1><b><?=$arResult["TICKET"]["TITLE"]?></b></h1>
...
<table class="support-ticket-edit data-table">
	...
	<tr>
		...
		<?if ($arResult["TICKET"]["DATE_CREATE"]!=$arResult["TICKET"]["TIMESTAMP_X"]):?>
				Modified: <?=$arResult["TICKET"]["TIMESTAMP_X"]?>
		<br />
		<?endif?>

		<!-- CATEGORY -->
		<?if (strlen($arResult["TICKET"]["CATEGORY_NAME"]) > 0):?>
				Category: <span title="<?=$arResult["TICKET"]["CATEGORY_DESC"]?>"><?=$arResult["TICKET"]["CATEGORY_NAME"]?></span><br />
		<?endif?>

		<!-- PRIORITY -->
		<?if(strlen($arResult["TICKET"]["CRITICALITY_NAME"])>0) :?>
				Priority: <span title="<?=$arResult["TICKET"]["CRITICALITY_DESC"]?>"><?=$arResult["TICKET"]["CRITICALITY_NAME"]?></span><br />
		<?endif?>

		<!-- RESPONSIBLE -->
		<?if(strlen($arResult["TICKET"]["RESPONSIBLE_NAME"])>0) :?>
				Responsible: <span><?=$arResult["TICKET"]["RESPONSIBLE_NAME"]?></span><br />
		<?endif?>
