...
$arResult['ENABLE_DISCOUNT'] = "Y"; /* JCW-15 */
	...
	)
	{
		if ($arProp['MULTIPLE'] == 'Y')
		{
			...
			if ($arProp['USER_TYPE'] !== 'HTML')
				$html .= '<input type="button" onclick="addNewTableRow(\'tbl'.$propID.'\', 1, /'.$propID.
					'\[(n)([0-9]*)\]/g, 2)" value="'.GetMessage('CRM_PRODUCT_PROP_ADD_BUTTON').'">';
...
$db_cat_list = CIBlockProperty::GetPropertyEnum("CATEGORY", Array(), Array());
$category_list = array();
while($category = $db_cat_list->Fetch()):
	$rsProd = CIBlockElement::GetList(array("SORT"=>"ASC"), array("ACTIVE"=>"Y", "PROPERTY_CATEGORY"=>$category["ID"]));
	$category_pids = array();
	while($prod = $rsProd->GetNext())
	{
		$category_pids[] = $prod["ID"];
	}

	$category_list[] = array("ID"=>$category["ID"], "NAME"=>$category["VALUE"], "PRODUCT_IDS"=>implode(",", $category_pids));
endwhile;
?>
...
?>
	<?#if ($bCanAddProduct):?>
	<div class="product_group_selector">
		<select name="category" id="grid_product_cat" class="crm-item-table-select">
			<option value="">Product Group</option>
			<? foreach($category_list as $cat): ?>
				<option value="<?=$cat["ID"]?>" data-pids="<?=$cat["PRODUCT_IDS"]?>"><?=$cat["NAME"]?></option>
			<? endforeach; ?>
		</select>
		<!--<span id="<?=$addProductBtnID?>_group" class="webform-small-button"><span class="webform-small-button-left"></span><span class="webform-small-button-text">Add</span><span class="webform-small-button-right"></span></span>-->
		<span id="add_product_button_group" class="webform-small-button"><span class="webform-small-button-left"></span><span class="webform-small-button-text">Add</span><span class="webform-small-button-right"></span></span>
	</div>
	<?# endif; ?>
...
?>
	<table id="<?= $productContainerID ?>" class="crm-items-table" style="<?= $nProductRows === 0 ? 'display: none;' : '' ?>">
		...
		for ($i = 0; $i < $nProductRows; $i++)
		{
			...
			// PRODUCT_NAME
			$htmlValues = array();
			$htmlValues['PART_NUM'] = $row["PART_NUM"];
			...
			<tr id="<?=$rowID?>" class="<?=$className?>">
				<td class="crm-item-cell crm-item-name">
					<span class="crm-item-cell-text"<?= ($bInitEditable && empty($fixedProductName)) ? '' : ' style="display: none;"' ?>>
						<span class="crm-table-name-left">
							...
							<input type="text" id="<?= ($rowID.'_PART_NUM') ?>" class="crm-item-table-inp crm-item-num-input" value="<?=$htmlValues['PART_NUM']?>" placeholder="Part #">
						</span>
				...
				<td class="crm-item-cell crm-item-price">
					<span class="crm-item-cell-text"<?= $bInitEditable ? '' : ' style="display: none;"' ?>>
						<input id="<?=$rowID.'_PRICE'?>" type="text" class="crm-item-table-inp" value="<?=$htmlValues['PRICE']?>" readonly/>
					</span>
				...
				<td class="crm-item-cell crm-item-tax-sum">
					<span class="crm-item-cell-text"<?= $bInitEditable ? '' : ' style="display: none;"' ?>>
						<div id="<?=$rowID.'_TAX_SUM'?>" class="crm-item-table-txt">0.0  <?=var_dump($bCanAddProduct)?></div>
					</span>
					<span class="crm-item-cell-view"<?= $bInitEditable ? ' style="display: none;"' : '' ?>>
						<div id="<?=$rowID.'_TAX_SUM_v'?>" class="crm-item-table-txt">0.0 <?=var_dump($bCanAddProduct)?></div>
					</span>
				</td>
				<?endif;?>
	...
	<?
	if ($enableCustomProducts && $bCanAddProduct):
			...
			<td class="crm-item-cell crm-item-name">
					<span class="crm-item-cell-text">
						<span class="crm-table-name-left">
							...
							<input type="text" id="<?= ($rowIdPrefix.'#N#_PART_NUM') ?>" class="crm-item-table-inp crm-item-num-input" value=""  autocomplete="off" placeholder="Part #">
						</span>
						<span class="crm-item-inp-wrap">
...
BX.ready(
	function()
	{
		...
		//trigger show discount click event
		document.getElementById('crm-top-sale-checkbox').checked = true;
