...
if (typeof(BX.CrmProductEditor) === "undefined")
{
	...
	BX.CrmProductEditor.prototype =
	{
		initialize: function (id, config)
		{
			...
			//add product by group
			var addBtnGroupId = document.getElementById('add_product_button_group');
				addBtnGroup = BX(addBtnGroupId);

			if(addBtnGroup) {
				BX.bind(
					addBtnGroup,
					"click",
					BX.delegate(this._handleAddBtnGroupClick, this)
				);
				this._topButtons.push(addBtnGroup);
			}
		...
		_handleAddBtnGroupClick: function()
		{
			var currencyID = this.getCurrencyId();
			var ajaxPageUrl = this.getSetting("productSearchUrl", "");

			var category_option = document.getElementById("grid_product_cat");
			var ar_product_id = category_option.options[category_option.selectedIndex].dataset.pids;

			if(ar_product_id != undefined){
				ar_product_id = ar_product_id.split(",");
				for(var key in ar_product_id){
					var productID = ar_product_id[key];
			...
			{
				alert("Invalid Request: Select product group.");
			}
		},
		...
		handleFormSubmit: function(e)
		{
			...
			/*
			//process custom ajax call
			//save part # to make_crm_product_row_part
			BX.ajax(
				{
					'url': '/bitrix/templates/.default/components/bitrix/crm.product_row.list/.default/product_row_ajax.php',
					'method': 'POST',
					'dataType': 'json',
					'data': 
					{
						"PRODUCT_ROWS": field.value
					},
			...
			//debug - prevent form submit reload
		...
		_prepareProductSettings: function(item, product)
		{
			...
			var itemPartNum = this.getSetting('PART_NUM', '');
		...
		_addItem: function(item, removeEmptyRow)
		{
			...
			var canAddProduct = this.getSetting("canAddProduct", false);
			if(!canAddProduct){
				price_inp = document.getElementById(row.id+ "_PRICE");
				price_inp.readOnly = true;
			}
			...
			if(item.id>0)
			{
				...
				BX.ajax(
				{
					...
					//onsuccess: BX.delegate(this.onProductPartSuccess, this),
					onsuccess: function(data)
					{
						part_num = document.getElementById(row.id+ "_PART_NUM");
						part_num.value = data.PART_NUM;						
					} 
				});
			}
		},
...
if (typeof(BX.CrmProduct) === "undefined")
{
	...
	BX.CrmProduct.prototype =
	{
		...
		layout: function()
		{
			...
			var productPart = this.getSetting("PART_NUM", "");
			...
			// PRODUCT PART #
			this.setFieldValue("PART_NUM", productPart);
		...
		saveSettings: function()
		{
			...
			this._settings['PART_NUM'] = BX.util.trim(this._fieldValue['PART_NUM']);
		...
		setSettings: function(settings)
		{
			...
			this._settings['PART_NUM'] = this.getSetting('PART_NUM', '');
		...
		setPartNum: function(value)
		{
			return this.setSetting('PART_NUM', parseInt(value));
		},
		...
		getPartNum: function()
		{
			return this.getSetting('PART_NUM', '');
		},
		...
		toJson: function()
		{
			...
			json += ", \"PART_NUM\":\"" + this.getSetting("PART_NUM", "") + "\"";
...
//				}
		...
		createInCatalogViaDialog: function(initialControl)
		{
			...
			customValues = {
				...
				"VAT_ID": taxId,
				"VAT_INCLUDED": taxIncluded,
				"PROPERTY_95": this.getSetting('PART_NUM', '')
			};
		...
		handleSetProductFromDialog: function(data)
		{
			...
			//set part# after add product to catalog
			var product = this;
			
			row = product._container;
			BX.ajax(
			{
				'url': '/bitrix/templates/.default/components/bitrix/crm.product_row.list/.default/product_row_ajax.php',
				'method': 'POST',
				...
				onsuccess: function(data)
				{
					product.setFieldValue("PART_NUM", data.PART_NUM, true);
				}
			});
		}
	};
