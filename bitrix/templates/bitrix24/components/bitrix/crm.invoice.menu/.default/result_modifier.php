...
if (is_callable(array('CSalePdf', 'isPdfAvailable')) && CSalePdf::isPdfAvailable()) {
    ...
    if (!is_null($replaceKey) && isset($arResult['BUTTONS'][ $replaceKey ])) {
        ...
        foreach ($languages as $lkey => $lang) {
            ...
            $path = '/crm/invoice/render.php?elementid='.intval($arParams['ELEMENT_ID']).'&request=render&pagemode=invoice&tempid=1&fileformat=pdf&targetfolderid=42'; // folder 42 = company drive/
            $path .= '&lang='.$lkey;
